class OnboardingController < ApplicationController
  ...
  def status
   ...
   def get_step1
    business_data = current_user.business&.slice(
      :name, :category, :year_established, :website, :about, :products_services
    ) || {}
    categories = Category.where(is_active: true).order(priority: :asc, name: :asc)
    render json: business_data.merge(available_categories: categories)
  end
